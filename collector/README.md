...
주요 이슈 선정 뒤에는 OpenAI Responses API를 한 이슈당 한 번 호출해 고정 카테고리와 대상 종목 기준 영향(`positive`, `negative`, `neutral`, `mixed`)을 구조화 출력으로 저장한다. 같은 제목을 제거하고 출처가 다른 대표 기사 최대 5개의 제목·요약만 입력한다. `출시`, `파업`, `실적`처럼 사건명에서 카테고리가 명확한 경우에는 규칙 guard로 LLM의 카테고리 오분류를 보정한다. 루트 `.env`에 다음 값을 설정한다.

```dotenv
OPENAI_API_KEY=...
STOCKECHO_LLM_MODEL=gpt-4.1-mini
STOCKECHO_LLM_TIMEOUT_SECONDS=30
```

API 키가 없거나 호출·형식 검증에 실패해도 Topic 분석은 중단하지 않는다. 이때 카테고리는 보수적인 키워드 규칙으로 채우고 영향은 `unknown`, 분류 방식은 `rule-fallback-v1`로 기록한다. LLM 성공 결과는 판단 신뢰도, 한 문장 근거, 근거 기사 ID, 모델명을 함께 저장한다. 화면에서는 신뢰도 70% 이상인 `positive`만 빨간색, `negative`만 파란색으로 표시하고 나머지는 회색으로 보수적으로 표시한다.
