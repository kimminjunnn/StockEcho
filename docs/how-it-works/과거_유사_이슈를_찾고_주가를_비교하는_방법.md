...
제거하고 상위 6개를 `coreKeywords`로 남긴다. 이 값은 NAVER 질의를
불필요하게 좁히지 않으면서, 저장 Event를 최종 선별할 때 추가 문맥 근거로
사용한다.
...
- 사건 검색어가 2개 이상이면 그중 최소 2개가 일치하는 Event
- 핵심어 점수 40% 이상인 Event
...
현재 유사도는 문장 임베딩이나 주가 패턴이 아니라 **사건 검색어·추가
문맥어·사건 유형·영향 방향·업종 힌트를 이용한 규칙 기반 점수**다.

```text
핵심어 점수 =
  전체 텍스트 사건 검색어 포함률 × 0.40
+ Event 이름·키워드 포함률 × 0.30
+ 대표 기사 제목·요약 포함률 × 0.15
+ Event에 없는 상위 Topic 문맥 포함률 × 0.05
+ 추가 coreKeywords 포함률 × 0.10

최종 규칙 점수 =
  핵심어 점수 × 나머지 가중치
+ 사건 유형 일치 × 0.15       # 양쪽에 유형이 있을 때
+ 영향 방향 일치 × 0.05       # 양쪽 방향이 known일 때
+ 업종 토큰 연관성 × 0.05     # 양쪽에 업종이 있을 때
```

사용할 수 없는 구조 특징의 가중치는 핵심어 점수로 돌아가므로, 과거 Event에
영향 방향이 없다는 이유만으로 감점하지 않는다.
...
예를 들어 현재 사건 검색어가 `성과급`, `협상`이고 추가 문맥어가 `노사`,
`결렬`일 때 두 사건 검색어가 모두 나타나면서 추가 문맥어까지 겹치는 Event가
먼저 선택된다. `성과급` 하나만 겹치는 Event는 최종 후보에서 제외한다.

`historical_events`에는 상위 Topic 이름·키워드와 Event 이름·키워드를 별도로
저장한다. 기존처럼 두 값이 동일한 행은 Topic 점수를 중복 가산하지 않는다.
업종은 다른 업종을 배제하는 필터가 아니라 동점에 가까운 후보를 정렬하는 약한
힌트로만 사용한다. 현재 주요 이슈의 `category`와 `impact`는 API가 저장된
canonical Event에서 읽어 Python에 전달한다. 과거 분석 snapshot의 분류값은
그대로 사용하고, Topic 및 NAVER proxy Event는 설명 가능한 규칙으로 사건
유형을 보완하되 영향 방향은 근거가 없으면 `unknown`으로 둔다.

따라서 화면 점수는 사건이 의미적으로 완전히 동일하다는 뜻이 아니라,
선택한 핵심어와 일부 문맥·사건 유형·영향 방향·업종 힌트가 과거 Event
자료와 일치하는 정도다. 기업 규모, 주가 상관관계와 D+ 수익률은 포함되지
않는다. 화면도 이를 의미 유사도와 구분되는 `규칙 유사도`로 표시한다.
...
가격 행을 날짜순으로 정렬한 뒤 기준 거래일의 다음 1·5·20번째 행을 찾는다.
...
아직 D+20 거래일에 도달하지 않았거나 가격을 가져오지 못하면 해당 값은
...
- 현재 Event 이름과 검색 핵심어
- 현재 Event 사건 유형과 영향 방향
- 최소 출처 수와 최소 유사도
- 전체 결과 수와 유사 종목 결과 수
- 현재 Event 제외 기간

동일 키 요청은 24시간 동안 NAVER와 KIS를 다시 호출하지 않고 `ready` 결과를
반환한다. 24시간이 지났거나 `historical_events`가 분석 결과보다 나중에
갱신됐으면 다시 분석한다.
동시에 같은 요청이 들어오는 경우 PostgreSQL advisory lock으로 한 요청만
분석을 수행한다.

NAVER backfill 자체도 schema version, 종목, 기준일, 검색 문구, 호출·페이지
제한, 최소 출처 수와 peer 범위를 포함한 `issue_search_<hash>` 파일로 24시간
캐시한다. 다만 제품의 반복 요청 방지는 Supabase 최종 결과 캐시가 우선한다.

현재 분석 schema는 `historical-issue-analysis-v8`이다. 검색·선별 규칙을
...
그래프는 D-0을 0%로 두고 실제로 확보한 D+1·D+5·D+20 관측점만
연결한다. 중간 날짜의 일별 경로를 추정하지 않는다. 심각도, 평균 회복 기간,
임의의 데이터 신뢰도처럼 현재 데이터로 계산할 수 없는 값도 표시하지 않는다.

모달 최하단의 예상 변동률은 확보된 과거 반응을 유사도와 원문 출처 수로
가중한 중앙값이며 p10–p90 범위를 함께 보여준다. `low/insufficient` 표본도
참고값은 표시하되 `탐색적 추정`으로 구분한다. 검증 기준 리밸런싱에는
사용하지 않으며, 참고용 시뮬레이션에서만 50% 중립값 쪽 보정과 종목별
변경폭 제한을 적용한다. 저장 forecast가 아직 없으면 저장 과거사례에서 같은
계산을 즉시 수행하며 외부 API나 Python 모델을 사용자 요청 중 실행하지 않는다.
...
| `historical_events` | 날짜·종목별 Topic/Event 문맥, 대표 기사, 기사 목록, 출처 수 |
...
현재 점수는 핵심어·문맥어, 사건 유형·알려진 영향 방향과 약한 업종 prior를
결합한 규칙 점수다. 의미 유사도를 높이려면 한국어 문장 임베딩을 별도
특징으로 계산하고 현재 점수와 결합해야 한다. 그전까지 UI 라벨은
`규칙 유사도`가 더 정확하다.
