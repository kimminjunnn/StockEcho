...
규칙은 특정 회사명에 종속시키지 않고 모든 종목에 동일하게 적용한다. 관련도 v2는 점수에 따라 `eligible`, `candidate`, `rejected`로 나누며 BERTopic에는 `eligible`만 전달한다. 회사명·검색 주제의 제목 등장은 가점하고 증시 종목 나열, 채용, 판촉, 수상, 혼합 뉴스 모음은 감점한다. 모든 가감 근거와 `relevance-v2` 규칙 버전을 저장한다.

```text
eligible  confidence >= 0.65
candidate 0.45 <= confidence < 0.65
rejected  confidence < 0.45
```

종목별 신규·무시 비율과 근거 유형 분포를 저장하여 자동화 품질을 비교한다.
